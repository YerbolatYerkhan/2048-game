...
public class MainView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        // 6. Комбо-текст (если ты его еще не удалил из onDraw)
        if (comboAlpha > 0) {
            paint.setTextAlign(Paint.Align.CENTER);
            paint.setTextSize(headerTextSize * 0.45f);
            paint.setColor(Color.parseColor("#F2B179"));
            paint.setAlpha((int) (comboAlpha * 255));
            paint.setFakeBoldText(true);
            float x = getWidth() / 2f;
            float y = startingY - dpToPx(40) + comboYOffset;
...
//            drawEndGameState(canvas);
    ...
    public void applyTheme(boolean isDark) {
        ...
        if (getWidth() > 0 && getHeight() > 0) {
            // Вызываем только создание фона, без лишних обновлений макета
        ...
        try {
            ...
            // 2. Обновляем фон игрового поля (квадрат под плитками)
            backgroundRectangle = safeGetDrawable(
                    resources,
                    isDark ? R.drawable.background_rectangle_dark : R.drawable.background_rectangle
            );

            // 3. Обновляем графику плиток
            for (int i = 0; i < cellRectangle.length; i++) {
                int value = (i == 0) ? 0 : (int) Math.pow(2, i);
                String name = (value == 0) ? "cell_rectangle" : "cell_rectangle_" + value;
                if (isDark) name += "_dark";

                int resId = resources.getIdentifier(name, "drawable", getContext().getPackageName());
                if (resId != 0) {
                    cellRectangle[i] = safeGetDrawable(resources, resId);
                } else {
                    cellRectangle[i] = safeGetDrawable(resources, R.drawable.cell_rectangle);
                }
            }
            ...
            // Внутри метода applyTheme, там где загружаются иконки:
            ...
            // 6. Обновляем цвета текста (критично для отрисовки цифр)
            ...
            TEXT_LIGHT_BLACK = ContextCompat.getColor(getContext(), R.color.text_light_black);

            // Цвет для инструкций и "2048"
            instructionTextColor = ContextCompat.getColor(
                    getContext(), isDark ? R.color.text_white : R.color.text_black
            );

            updatePaintColors();

            // 7. Фон самого View (чтобы не было белых краев вокруг игры)
            int bgColor = ContextCompat.getColor(getContext(),
                    isDark ? R.color.background_dark : R.color.background_light);
            setBackgroundColor(bgColor);

            // 🔥 КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Перерисовываем статичные элементы 🔥
            // Без этого заголовок "2048" и инструкции не изменят цвет
            if (getWidth() > 0 && getHeight() > 0) {
                createBackgroundBitmap(getWidth(), getHeight());
            }

            createBitmapCells(); // Перерисовываем цифры на плитках
            createOverlays();    // Перерисовываем экран победы/проигрыша

            invalidate(); // Принудительно перерисовываем экран
