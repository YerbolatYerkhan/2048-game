...
public class MainGame {
    ...
    public void newGame() {
        // 1. Если сетка уже существует, сохраняем её для отката (Undo)
        if (grid != null) {
            prepareUndoState();
            ...
            // 2. Если это ПЕРВЫЙ запуск (grid еще null), создаем сетку
            grid = new Grid(numSquaresX, numSquaresY);
        }

        // ВАЖНО: Всегда инициализируем сетку анимаций, чтобы не было NullPointerException
        aGrid = new AnimationGrid(numSquaresX, numSquaresY);

        // 3. Сбрасываем параметры
        score = 0;
        gameState = GAME_NORMAL;
        ...
        // 4. Добавляем плитки (теперь aGrid не null, и вылета не будет)
        addStartTiles();

        canUndo = true; // Разрешаем откат
    ...
    private void playSound(int id) {
        // 1. Проверяем настройки звука из SharedPreferences
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mContext);
        boolean isSoundEnabled = prefs.getBoolean("sound_enabled", true); // "sound_enabled" должен совпадать с ключом в MainActivity

        if (!isSoundEnabled) {
            return; // Если выключено — выходим из метода
        }

        // 2. Если включено, играем звук
        AudioManager audio = (AudioManager) mView.getContext()
        ...
        if (soundMap.containsKey(id)) {
            soundPool.play(soundMap.get(id), volume, volume, 1, 0, 1f);
        }
    }
}
