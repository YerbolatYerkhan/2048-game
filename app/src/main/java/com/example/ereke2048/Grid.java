...
public class Grid {
    ...
    // Добавь это в класс Grid (или там, где у тебя массив field)
    public boolean clearSmallTiles() {
        boolean removed = false;
        for (int x = 0; x < field.length; x++) {
            for (int y = 0; y < field[0].length; y++) {
                if (field[x][y] != null) {
                    int value = field[x][y].getValue();
                    // Проверяем, является ли плитка "мелкой"
                    if (value == 2 || value == 4) {
                        field[x][y] = null; // Удаляем
                        removed = true;
                    }
                }
            }
        }
        return removed; // Возвращаем результат: удалили мы что-то или нет
    }
}
